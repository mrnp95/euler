...
def make_system(type_4band = 'monolayer'):
    ...
    bulk[B_sub_A(0,0)] = 0.4036+epsA*0+delta
    bulk[B_sub_B(0,0)] = 0.4036+epsB*0-delta
    bulk[B_sub_C(0,0)] = -0.4036+epsC*0-delta
    bulk[B_sub_D(0,0)] = -0.4036+epsD*0+delta
...
def symm_point(text:str):
    ...
    elif  text == 'M': return np.array([np.pi,0])
...
def k_path_bandstructure(sys, *args):
    ...
    for n in (np.array(energy)).T: 
        plt.plot(dummy, n)
    plt.savefig("11_nodal_Section"+str("{:.2f}".format(x))+".png")
    #plt.show()
...
for x in np.linspace(0.50, 0.50, 1): 
    ...
    ax.savefig('11_nodal_BS'+str("{:.2f}".format(x))+'.png')
