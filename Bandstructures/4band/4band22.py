...
def make_system(type_4band = 'monolayer'):
    ...
    bulk[kwant.builder.HoppingKind((1,1), B_sub_A,B_sub_D)] = -t2
    bulk[kwant.builder.HoppingKind((1,-1), B_sub_A,B_sub_D)] = -t2*0
    bulk[kwant.builder.HoppingKind((-1,-1), B_sub_A,B_sub_D)] = -t2
    bulk[kwant.builder.HoppingKind((-1,1), B_sub_A,B_sub_D)] = -t2*0
     
    bulk[kwant.builder.HoppingKind((1,1), B_sub_B,B_sub_C)] = t2
    bulk[kwant.builder.HoppingKind((1,-1), B_sub_B,B_sub_C)] = t2*0
    bulk[kwant.builder.HoppingKind((-1,-1), B_sub_B,B_sub_C)] = t2
    bulk[kwant.builder.HoppingKind((-1,1), B_sub_B,B_sub_C)] = t2*0   
...
def k_path_bandstructure(sys, *args):
    ...
    for n in (np.array(energy)).T: 
        plt.plot(dummy, n)
    plt.savefig("22Section"+str("{:.2f}".format(x))+".png")
    #plt.show()
...
for x in np.linspace(0.50, 0.50, 1): 
    ...
    ax.savefig('22BS'+str("{:.2f}".format(x))+'.png')
