...
for x in [0.00, 0.28, 0.33, 0.50, 1.00]:
    ...
    for d in [0.01, 0.1, 0.3, 0.5, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 3.0]:
        ...
        legend_combined.append("$\sigma_{xx}$/4, $\sigma$ = "+str("{:.2f}".format(d)))
        legend_combined.append("$\sigma_{xy}$, $\sigma$ = "+str("{:.2f}".format(d)))
     ...
     # Conductivity combined
        ...
        ax_cond_combined.set_title("Combined average conductivity at different chemical potentials \n (t = "+str(round(t1, 2))+", t' = "+str(round(t2, 2))+")", y=1.1)
    ...
    fig_dos.savefig(save_fig_to + "Euler_DOS_dis_full_tNNN_"+str(x)+".png", dpi='figure')
    fig_cond_xx.savefig(save_fig_to + "Euler_Sigma_xx_mu_tNNN_"+str(x)+".png", bbox_inches = 'tight', dpi='figure')
    fig_cond_xy.savefig(save_fig_to + "Euler_Sigma_xy_mu_tNNN_"+str(x)+".png", bbox_inches = 'tight', dpi='figure')
    fig_cond_combined.savefig(save_fig_to + "Euler_Sigma_combined_mu_tNNN_"+str(x)+".png", bbox_inches = 'tight', dpi='figure')
