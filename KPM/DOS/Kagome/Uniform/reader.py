...
for x in [0.00, 0.28, 0.33, 0.50, 1.00]:
    ...
    fig_cond_combined, ax_cond_combined = plt.subplots()
    ...
    for d in [0.01, 0.1, 0.3, 0.5, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 3.0]:
        ...
        save_fig_to = './'
        legend.append("$V_0$ = "+str("{:.2f}".format(d)))
        legend_combined.append("$\sigma_{xx}$/4, $V_0$ = "+str("{:.2f}".format(d)))
        legend_combined.append("$\sigma_{xy}$, $V_0$ = "+str("{:.2f}".format(d)))
     ...
     # Conductivity combined
        ...
        ax_cond_combined.plot(est_energies, np.array(est_cond_xx_miu, np.complex64)/4,'-')
        ax_cond_combined.plot(est_energies, np.array(est_cond_xy_miu, np.complex64), '--') 
        ax_cond_combined.set_title("Combined average conductivities at different chemical potentials \n (t = "+str(round(t1, 2))+", t' = "+str(round(t2, 2))+")", y=1.1)
        ax_cond_combined.set_xlabel(r"Chemical potential $\mu$")
        ax_cond_combined.set_ylabel(r"Conductivity $\langle \sigma \rangle$ $(e^2/h)$")
        
    ax_dos.set_xlim(e_ax_min, e_ax_max)
    ax_cond_xx.set_xlim(e_ax_min, e_ax_max)
    ax_cond_xy.set_xlim(e_ax_min, e_ax_max)
    ax_cond_combined.set_xlim(e_ax_min, e_ax_max)
    ax_dos.set_ylim(0, 1.0)
    fig_dos.legend(legend, loc='upper left', bbox_to_anchor=(0.15, 0.84))
    ...
    fig_cond_combined.legend(legend_combined, loc='center left', bbox_to_anchor=(1.00, 0.50), prop={'size': 8})
    fig_dos.tight_layout() 
    fig_cond_xx.tight_layout() 
    fig_cond_xy.tight_layout() 
    fig_cond_combined.tight_layout() 
    fig_dos.savefig(save_fig_to + "Uniform_Euler_DOS_dis_full_tNNN_"+str(x)+".png", dpi='figure')
    fig_cond_xx.savefig(save_fig_to + "Uniform_Euler_Sigma_xx_mu_tNNN_"+str(x)+".png", bbox_inches = 'tight', dpi='figure')
    fig_cond_xy.savefig(save_fig_to + "Uniform_Euler_Sigma_xy_mu_tNNN_"+str(x)+".png", bbox_inches = 'tight', dpi='figure')
    fig_cond_combined.savefig(save_fig_to + "Uniform_Euler_Sigma_combined_mu_tNNN_"+str(x)+".png", bbox_inches = 'tight', dpi='figure')
