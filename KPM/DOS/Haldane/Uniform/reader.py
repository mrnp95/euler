...
for x in [0.00, 0.50]:
    ...
    for d in [0.01, 0.1, 0.3, 0.5, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 3.0]:
        ...
        ax_dos.set_title("Disordered Haldane DOS with KPM (|t| = "+str(round(abs(t1), 2))+", |t'| = "+str(round(abs(t2), 2))+")", y=1.1)
    ...
    ax_dos.set_ylim(0, 0.4)
    ax_cond_xy.set_ylim(-0.1, 1.2)
    ...
    fig_dos.savefig(save_fig_to + "Uniform_Haldane_DOS_dis_full_tNNN_"+str(x)+".png", bbox_inches = 'tight', dpi='figure')
    fig_cond_xx.savefig(save_fig_to + "Uniform_Haldane_Sigma_xx_mu_tNNN_"+str(x)+".png", bbox_inches = 'tight', dpi='figure')
    fig_cond_xy.savefig(save_fig_to + "Uniform_Haldane_Sigma_xy_mu_tNNN_"+str(x)+".png", bbox_inches = 'tight', dpi='figure')
