...
for x in [0.00, 0.28, 0.33, 0.50, 1.00]:
    ...
    fig_cond_xx.savefig(save_fig_to + "Gaussian_Scaling_Euler_Sigma_xx_tNNN_"+str(x)+"_miu_"+str(miu)+".png", bbox_inches = 'tight', dpi='figure')
    
    fig_cond_xx_log.legend(legend, loc='upper right', bbox_to_anchor=(0.90, 0.8))       
    fig_cond_xx_log.tight_layout()
    fig_cond_xx_log.savefig(save_fig_to + "Log_Gaussian_Scaling_Euler_Sigma_xx_tNNN_"+str(x)+"_miu_"+str(miu)+".png", bbox_inches = 'tight', dpi='figure')
