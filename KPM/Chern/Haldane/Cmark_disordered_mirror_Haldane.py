...
def make_system(type_graphene = 'monolayer'):
    ...
    Bottom_Lat_pos = [(0, lat_const/sqrt(3),  0), (0,  0,  0)]    # The position of sublattice atoms in the layer  
    ...
    bulk[kwant.builder.HoppingKind((0,0,0), B_sub_a,B_sub_b)] = t1
    bulk[kwant.builder.HoppingKind((0,-1,0), B_sub_a,B_sub_b)] = t1
    bulk[kwant.builder.HoppingKind((1,-1,0), B_sub_a,B_sub_b)] = t1
    
        # Next neighbors

    bulk[kwant.builder.HoppingKind((1,0,0), B_sub_b,B_sub_b)] = -t2
    bulk[kwant.builder.HoppingKind((0,1,0), B_sub_b,B_sub_b)] = -t2
    bulk[kwant.builder.HoppingKind((1,-1,0), B_sub_b,B_sub_b)] = -t2 
    bulk[kwant.builder.HoppingKind((1,0,0), B_sub_a,B_sub_a)] = t2
    bulk[kwant.builder.HoppingKind((0,1,0), B_sub_a,B_sub_a)] = t2
    bulk[kwant.builder.HoppingKind((1,-1,0), B_sub_a,B_sub_a)] = t2
...
for t in hoppings:
    ...
    for d in disorders:
        ...
        for av in range(0, averaging):
            ...
            tags = np.array([np.hstack([np.random.randint(-int(L/2)+25, high=(int(L/2)-25), size=2), 0]) for i in range(0, num_vectors)])
            vectors = []
            Cs = 0
            C_all = []
            err = 0
    ...
    ax_C.set_title("Averaged local mirror Chern number $C_{M}$ as a function of disorder \n (|t| = "+str(round(np.abs(t1), 2))+", |t'| = "+str(round(np.abs(t2), 2))+")", y=1.1)
    ...
    fig_C.tight_layout()
    save_fig_to = './'
    fig_C.savefig(save_fig_to + "Mirror_marking_Haldane_tNNN_"+str(t)+".png", bbox_inches = 'tight', dpi='figure')

    ############
    # SAVING   #
    ############

    file_name = "Haldane_Chern_Mirror_marking_dis_full_"+"_tNNN_"+str(t)+"_L_"+str(L)+".dat"
