...
disorders = [0.3, 0.5, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 3.0, 4.0, 5.0]
hoppings = [1.00, 0.28, 0.33, 0.50, 1.00]
...
def make_system(type_kagome = 'monolayer'):
    ...
    Bottom_Lat_pos = [(0.5*lat_const*0, 0, 0), (0.25*lat_const*0, 0.25*lat_const*sqrt(3), 0), (0,  0, 0)]    # The position of sublattice atoms in Bottom layer  
    ...
    for x in range(int(-3*L/2), int(3*L/2)):
        for y in range(int(-3*L/2), int(3*L/2)):

	    # define hopping and on-site potential on bottom layer kagome

            bulk[B_sub_a(x,y,0)] = np.random.normal(0, d, 1)
            bulk[B_sub_b(x,y,0)] = np.random.normal(0, d, 1)
            bulk[B_sub_c(x,y,0)] = np.random.normal(0, d, 1)
    ...
    bulk[kwant.builder.HoppingKind((1,0,0), B_sub_a,B_sub_a)] = t3
    bulk[kwant.builder.HoppingKind((0,1,0), B_sub_a,B_sub_a)] = t3
    bulk[kwant.builder.HoppingKind((-1,1,0), B_sub_a,B_sub_a)] = t3
    
    bulk[kwant.builder.HoppingKind((1,0,0), B_sub_b,B_sub_b)] = t3
    bulk[kwant.builder.HoppingKind((0,1,0),B_sub_b,B_sub_b)] = t3
    bulk[kwant.builder.HoppingKind((-1,1,0), B_sub_b,B_sub_b)] = t3

    bulk[kwant.builder.HoppingKind((1,0,0), B_sub_c,B_sub_c)] = t3
    bulk[kwant.builder.HoppingKind((0,1,0), B_sub_c,B_sub_c)] = t3
    bulk[kwant.builder.HoppingKind((-1,1,0), B_sub_c,B_sub_c)] = t3
...
for t in hoppings:
    ...
    for d in disorders:
        ...
        for av in range(0, averaging):
            ...
            for tag in tags:
                ...
                C_list = ch.mirror_chern(fsyst, x, y, Mz=M, vectors=vectors, e_F=miu, kpm_params=dict(num_moments=num_moments), params=None, bounds=None, window=None, return_std=False)
    ...
    ax_C.set_title("Averaged local Chern number C as a function of disorder \n (|t| = "+str(round(np.abs(t1), 2))+", |t'| = "+str(round(np.abs(t2), 2))+", |t''| = "+str(round(np.abs(t3), 2))+")", y=1.1)
    ...
    fig_C.tight_layout()
    save_fig_to = './'
    fig_C.savefig(save_fig_to + "Mirror_marking_Euler_tNNNN_"+str(t)+".png", bbox_inches = 'tight', dpi='figure')

    ############
    # SAVING   #
    ############

    file_name = "Mirror_Euler_Chern_marking_dis_full_"+"_tNNNN_"+str(t)+"_L_"+str(L)+".dat"
