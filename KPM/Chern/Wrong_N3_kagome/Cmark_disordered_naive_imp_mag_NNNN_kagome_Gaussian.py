...
disorders = [5.0, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 3.0, 4.0, 5.0]
hoppings = [0.28, 0.33, 0.50, 1.00]
...
def make_system(type_kagome = 'monolayer'):
    ...
    bulk[kwant.builder.HoppingKind((0,1), B_sub_c,B_sub_a)] = t2+(1-1/(d*d+1))*1j
    bulk[kwant.builder.HoppingKind((1,-1), B_sub_c,B_sub_a)] = t2+(1-1/(d*d+1))*(-1j)
    #bulk[kwant.builder.HoppingKind((1,0), B_sub_b,B_sub_a)] = t2
    #bulk[kwant.builder.HoppingKind((0,-1), B_sub_b,B_sub_a)] = t2
    bulk[kwant.builder.HoppingKind((-1,0), B_sub_a,B_sub_b)] =  t2+(1-1/(d*d+1))*1j
    bulk[kwant.builder.HoppingKind((0,1),B_sub_a,B_sub_b)] =  t2+(1-1/(d*d+1))*(-1j)
    #bulk[kwant.builder.HoppingKind((1,0), B_sub_c,B_sub_b)] = t2
    #bulk[kwant.builder.HoppingKind((-1,1), B_sub_c,B_sub_b)] = t2
    bulk[kwant.builder.HoppingKind((1,-1), B_sub_b,B_sub_c)] =  t2+(1-1/(d*d+1))*1j  
    bulk[kwant.builder.HoppingKind((-1,0), B_sub_b,B_sub_c)] = t2+(1-1/(d*d+1))*1j
...
for t in hoppings:
    ...
    for d in disorders:
        ...
        for av in range(0, averaging):
            ...
            tags = np.array([np.random.randint(-int(L/2), high=(int(L/2)), size=2) for i in range(0, num_vectors)])
            vectors = []
            Cs = 0
            C_all = []
            err = 0
    ...
    fig_C.tight_layout()
    save_fig_to = './'
    fig_C.savefig(save_fig_to + "Marking_Euler_tNNNN_"+str(t)+".png", bbox_inches = 'tight', dpi='figure')

    ############
    # SAVING   #
    ############

    file_name = "Euler_Chern_marking_dis_full_"+"_tNNNN_"+str(t)+"_L_"+str(L)+".dat"
