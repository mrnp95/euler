...
def make_system(type_kagome = 'monolayer'):
    ...
    phase = lambda: np.exp(1j*np.random.uniform(-np.pi, np.pi))
    ...
    for x in range(int(-L/2), int(L/2)+1):
        for y in range(int(-L/2), int(L/2)+1): 
            ...
            bulk[B_sub_b(x,y), B_sub_a(x,y)] = t1 + rn(d)*phase()
            bulk[B_sub_c(x,y), B_sub_b(x,y)] = t1 + rn(d)*phase()
            bulk[B_sub_a(x,y), B_sub_c(x,y)] = t1 + rn(d)*phase()
            ...
            if cx1:
                bulk[B_sub_a(x-1,y), B_sub_c(x,y)] = t1 + rn(d)*phase()
            if cy2:
                bulk[B_sub_c(x,y+1), B_sub_b(x,y)] = t1 + rn(d)*phase()
            if (cx2 and cy1):
                bulk[B_sub_b(x+1,y-1), B_sub_a(x,y)] = t1 + rn(d)*phase()
    
            
            # Next neighbors
            
            if cy2:
                bulk[B_sub_c(x,y+1),B_sub_a(x,y)] = t2 + rn(d)*phase()
            if (cx2 and cy1):
                bulk[B_sub_c(x+1,y-1),B_sub_a(x,y)] = t2 + rn(d)*phase()
            if cx1:
                bulk[B_sub_a(x-1,y),B_sub_b(x,y)] =  t2 + rn(d)*phase()
            if cy2:
                bulk[B_sub_a(x,y+1),B_sub_b(x,y)] =  t2 + rn(d)*phase()
            if (cx2 and cy1):
                bulk[B_sub_b(x+1,y-1),B_sub_c(x,y)] =  t2 + rn(d)*phase()
            if cx1:
                bulk[B_sub_b(x-1,y),B_sub_c(x,y)] = t2 + rn(d)*phase()

            # Next nearest neighbors
            
            if cx2:
                bulk[B_sub_a(x+1,y),B_sub_a(x,y)] = t3 + rn(d)*phase()
            if cy2:
                bulk[B_sub_a(x,y+1),B_sub_a(x,y)] = t3 + rn(d)*phase()
            if (cx1 and cy2):
                bulk[B_sub_a(x-1,y+1),B_sub_a(x,y)] = t3 + rn(d)*phase()
            if cx2:
                bulk[B_sub_b(x+1,y),B_sub_b(x,y)] = t3 + rn(d)*phase()
            if cy2:
                bulk[B_sub_b(x,y+1),B_sub_b(x,y)] = t3 + rn(d)*phase()
            if (cx1 and cy2):
                bulk[B_sub_b(x-1,y+1),B_sub_b(x,y)] = t3 + rn(d)*phase()
            if cx2:
                bulk[B_sub_c(x+1,y),B_sub_c(x,y)] = t3 + rn(d)*phase()
            if cy2:
                bulk[B_sub_c(x,y+1),B_sub_c(x,y)] = t3 + rn(d)*phase()
            if (cx1 and cy2):
                bulk[B_sub_c(x-1,y+1),B_sub_c(x,y)] = t3 + rn(d)*phase()
...
for t in hoppings:
    ...
    fig_C.tight_layout()
    save_fig_to = './'
    fig_C.savefig(save_fig_to + "Marking_mag_imp_Euler_tNNNN_"+str(t)+".png", bbox_inches = 'tight', dpi='figure')

    ############
    # SAVING   #
    ############

    file_name = "Euler_mag_imp_Chern_marking_dis_full_"+"_tNNNN_"+str(t)+"_L_"+str(L)+".dat"
