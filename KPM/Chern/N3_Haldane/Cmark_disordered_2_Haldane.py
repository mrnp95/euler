...
t2 = 1.0   # next nearest neighbor hopping parameter for graphene (unit: eV)
t3 = 0.5 # N3 hopping
m = -0.0 # mass parameter
...
def make_system(type_graphene = 'monolayer'):
    ...
    bulk[kwant.builder.HoppingKind((1,0), B_sub_b,B_sub_b)] = -t2*1j
    bulk[kwant.builder.HoppingKind((0,1), B_sub_b,B_sub_b)] = -t2*1j
    bulk[kwant.builder.HoppingKind((1,-1), B_sub_b,B_sub_b)] = -t2*1j 
    bulk[kwant.builder.HoppingKind((1,0), B_sub_a,B_sub_a)] = t2*1j
    bulk[kwant.builder.HoppingKind((0,1), B_sub_a,B_sub_a)] = t2*1j
    bulk[kwant.builder.HoppingKind((1,-1), B_sub_a,B_sub_a)] = t2*1j
    
        # N3 hoppings
    
    bulk[kwant.builder.HoppingKind((1,0), B_sub_a,B_sub_b)] = t3
    bulk[kwant.builder.HoppingKind((1,-2), B_sub_a,B_sub_b)] = t3
    bulk[kwant.builder.HoppingKind((-1,0), B_sub_a,B_sub_b)] = t3 
    bulk[kwant.builder.HoppingKind((-1,0), B_sub_b,B_sub_a)] = t3
    bulk[kwant.builder.HoppingKind((1,-2), B_sub_b,B_sub_a)] = t3
    bulk[kwant.builder.HoppingKind((1,0), B_sub_b,B_sub_a)] = t3
...
for t in [0.35, 0.00]:
    ...
    fig_C.tight_layout()
    save_fig_to = './'
    fig_C.savefig(save_fig_to + "Marking_N3Haldane_tNNNN_"+str(t)+".png", bbox_inches = 'tight', dpi='figure')
	
	############
    # SAVING   #
    ############

    file_name = "N3Haldane_Chern_marking_dis_full_"+"_tNNNN_"+str(t)+"_L_"+str(L)+".dat"
