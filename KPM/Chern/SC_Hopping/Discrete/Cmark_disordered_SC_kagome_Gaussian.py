...
#scalings = [np.pi*(2/3), np.pi/2, np.pi/3, np.pi/4, np.pi/5, np.pi/6, np.pi/6, np.pi/5 , np.pi/4, np.pi/3, np.pi/2, np.pi*(2/3), np.pi] 
...
def make_system(type_kagome = 'monolayer'):
    ...
    phase_cc = lambda x1,y1,x2,y2: np.exp(sc*1j*np.sign(bulk[B_sub_c(x1,y1)]-bulk[B_sub_c(x2,y2)]))
    phase_bc = lambda x1,y1,x2,y2: np.exp(sc*1j*np.sign(bulk[B_sub_b(x1,y1)]-bulk[B_sub_c(x2,y2)]))
    phase_ac = lambda x1,y1,x2,y2: np.exp(sc*1j*np.sign(bulk[B_sub_a(x1,y1)]-bulk[B_sub_c(x2,y2)]))
    phase_ab = lambda x1,y1,x2,y2: np.exp(sc*1j*np.sign(bulk[B_sub_a(x1,y1)]-bulk[B_sub_b(x2,y2)]))
    phase_cb = lambda x1,y1,x2,y2: np.exp(sc*1j*np.sign(bulk[B_sub_c(x1,y1)]-bulk[B_sub_b(x2,y2)]))
    phase_ba = lambda x1,y1,x2,y2: np.exp(sc*1j*np.sign(bulk[B_sub_b(x1,y1)]-bulk[B_sub_a(x2,y2)]))
    phase_ca = lambda x1,y1,x2,y2: np.exp(sc*1j*np.sign(bulk[B_sub_c(x1,y1)]-bulk[B_sub_a(x2,y2)]))
...
for t in hoppings:
    ...
    for theta in thetas:
        ...
        legend.append("{:.2f}".format(theta/(2*np.pi)))
