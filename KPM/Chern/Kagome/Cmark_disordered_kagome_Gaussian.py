...
def make_system(type_kagome = 'monolayer'):
    ...
    Bottom_Lat_pos = [(0.25*lat_const*0, 0, 0), (-0.25*lat_const*0, 0, 0), (0, 0.25*lat_const*sqrt(3), 0)]    # The position of sublattice atoms in Bottom layer  
    ...
    for x in range(int(-3*L/2), int(3*L/2)):
	    ...
	    # define hopping and on-site potential on bottom layer kagome
            ...
            bulk[B_sub_a(x,y,0)] = np.random.normal(-1, d, 1)
            bulk[B_sub_b(x,y,0)] = np.random.normal(-1, d, 1)
            bulk[B_sub_c(x,y,0)] = np.random.normal(-1, d, 1)
...
for t in hoppings:
    ...
    fig_C.tight_layout()
    save_fig_to = './'
    fig_C.savefig(save_fig_to + "Mirror_marking_-1shifted__Euler_tNNN_"+str(t)+".png", bbox_inches = 'tight', dpi='figure')

    ############
    # SAVING   #
    ############

    file_name = "Euler_Chern_Mirror_-1shifted_marking_dis_full_"+"_tNNN_"+str(t)+"_L_"+str(L)+".dat"
