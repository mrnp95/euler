...
def make_system(epsA, epsB, epsC, epsD, delta, lat_const, L, d, type_4band='monolayer'):
    ...
    for x in range(int(-L / 2), int(L / 2)):
        for y in range(int(-L / 2), int(L / 2)):
            bulk[B_sub_A(x, y)] = 0.4036 + epsA * 0 + delta + np.random.normal(0, d, 1)
            bulk[B_sub_B(x, y)] = 0.4036 + epsB * 0 - delta + np.random.normal(0, d, 1)
            bulk[B_sub_C(x, y)] = -0.4036 + epsC * 0 - delta + np.random.normal(0, d, 1)
            bulk[B_sub_D(x, y)] = -0.4036 + epsD * 0 + delta + np.random.normal(0, d, 1)
...
def cluster_run(run_index):
    ...
    for av in range(0, averaging):
        ...
        syst = kwant.Builder()
        model = make_system(epsA, epsB, epsC, epsD, delta, lat_const, L, d, type_4band='monolayer')
