...
def make_system(type_4band = 'monolayer'):
    ...
    for x in range(int(-L/2), int(L/2)):
        for y in range(int(-L/2), int(L/2)):
            ...
            bulk[B_sub_D(x,y)] = np.random.normal(0, d, 1)
    ...
    bulk[kwant.builder.HoppingKind((0,1), B_sub_A,B_sub_D)] = -t1/2
    bulk[kwant.builder.HoppingKind((0,-1), B_sub_A,B_sub_D)] = -t1/2
    bulk[kwant.builder.HoppingKind((1,0), B_sub_A,B_sub_D)] = -t1/2
    bulk[kwant.builder.HoppingKind((-1,0), B_sub_A,B_sub_D)] = -t1/2
     
    bulk[kwant.builder.HoppingKind((0,1), B_sub_B,B_sub_C)] = t1/2
    bulk[kwant.builder.HoppingKind((0,-1), B_sub_B,B_sub_C)] = t1/2
    bulk[kwant.builder.HoppingKind((1,0), B_sub_B,B_sub_C)] = t1/2
    bulk[kwant.builder.HoppingKind((-1,0), B_sub_B,B_sub_C)] = t1/2   

    bulk[kwant.builder.HoppingKind((1,1), B_sub_A,B_sub_D)] = -t2/2
    bulk[kwant.builder.HoppingKind((1,-1), B_sub_A,B_sub_D)] = 0
    bulk[kwant.builder.HoppingKind((-1,-1), B_sub_A,B_sub_D)] = -t2/2
    bulk[kwant.builder.HoppingKind((-1,1), B_sub_A,B_sub_D)] = 0
     
    bulk[kwant.builder.HoppingKind((1,1), B_sub_B,B_sub_C)] = t2/2
    bulk[kwant.builder.HoppingKind((1,-1), B_sub_B,B_sub_C)] = 0
    bulk[kwant.builder.HoppingKind((-1,-1), B_sub_B,B_sub_C)] = t2/2
    bulk[kwant.builder.HoppingKind((-1,1), B_sub_B,B_sub_C)] = 0   
...
for x in [1.00, 0.00]:
    ...
    for d in [0.01, 0.1, 0.3, 0.5, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 3.0, 4.0, 5.0]:
        ...
        file_name = "4band22_DOS_dis_full_"+str(d)+"_m_"+str(x)+"_L_"+str(L)+".dat"
